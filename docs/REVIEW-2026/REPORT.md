...
| 11 | **Phone/mobile registration didn't work** — the "sign up with your mobile for tips and offer drops" card produces no result | BUG / FEAT | Confirmed two baked-in failure modes in `app/api/profile/mobile/route.ts`: (a) **guests silently no-op** — route returns `{ ok:true, persisted:false }`, number kept only in `localStorage`, never saved; (b) **no downstream dispatch is wired** ("Outbound Hermes/Telegram dispatch is intentionally not wired here"; WhatsApp hard-locked in `.env.example`) — so even a saved number sends nothing | Short term: persist for guests (or gate the card behind sign-in) and give clear success/failure feedback; don't promise "offer drops" until a dispatch worker exists. Medium term: wire the actual notification channel as part of the returning-user loop (§B5 data-decay/refresh prompts are the first real use) | **High** (broken promise on a primary CTA) |
